...
const GuruFilter: NextPage = ({ token }: any) => {
  ...
  const isCademi =
    Object.keys(query).filter((key) => {
      return key.includes('cademi');
    }) || [];

  if (isCademi.length > 0) {
    const cademi = query[isCademi[0]];
    console.log(cademi);
    fetch('https://criptomaniacos.cademi.com.br/api/postback/custom', {
      method: 'POST',
      headers: {
...
export async function getStaticProps({ params }: any) {
  const { token } = params;
  return {
    props: { token, cademiToken: process.env.CADEMI_TOKEN }, // will be passed to the page component as props
  };
}
